...
/**
 *
 * 解析redis的dump.rdb文件, 可以支持最新的RDB第6版本 RBD文件格式可以参照 :
...
public class ParseRDB {
    ...
    String loadStringObject() {
        ...
        if (isencoded.get()) {
            ...
            switch ((int) len) {
                ...
                case REDIS_RDB_ENC_LZF://文档是4，实测是3
    ...
    byte[] loadStringObjectBytes() {
        ...
        if (isencoded.get()) {
            ...
            switch ((int) len) {
                case REDIS_RDB_ENC_LZF://实际证明是3，文档是4
    ...
    Entry loadEntry() {
        ...
        if (e.type == REDIS_SELECTDB) {
            ...
            if (e.type == REDIS_EXPIRETIME_FD || e.type == REDIS_EXPIRETIME_FC) {
                byte[] buf =null; //FD应该是4吧？
                if(e.type == REDIS_EXPIRETIME_FD){
                	buf=processTime(4);
                }else{
                	buf=processTime(8);
                }
                if (buf == null) {
                    ...
                    long expire =  ((long) (buf[3] & 0x00ff) << 24)
                            + ((long) (buf[2] & 0x00ff) << 16) + ((long) (buf[1] & 0x00ff) << 8)
                            + ((long) (buf[0] & 0x00ff));
                    if(e.type==REDIS_EXPIRETIME_FC){
                    	expire+=(((long) (buf[7] & 0x00ff) << 56)
                        + ((long) (buf[6] & 0x00ff) << 48) + ((long) (buf[5] & 0x00ff) << 40)
                        + ((long) (buf[4] & 0x00ff) << 32) );
                    }
